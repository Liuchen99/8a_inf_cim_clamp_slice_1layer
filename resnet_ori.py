...
class BasicBlock_1w8a_q(nn.Module):
    ...
    def my_bn1(self, input, sw_conv1, err):
        global var, mean
        if self.training:
            y = input
            y = y.permute(1, 0, 2, 3)
        ...
        T_a = 7 * 3 * 3 * self.conv1.weight.shape[1]
        T_w = 1

        bw, T_w = self.my_quantize_7(weight, 3)

        activation_q = torch.floor((torch.round(input * 1023 / T_a) + err) * sw_conv1)

        bb = torch.round(self.my_quantize(bias, 14) * 14 * 1023 / T_a * 7 / T_w)

        out = F.conv2d(activation_q, bw, bb, stride=1, padding=0)

        return out, T_a, T_w

    def my_bn2(self, input, sw_conv2, err2):
        global var2, mean2
        if self.training:
            y2 = input
        ...
        T_w2 = 1
        T_a2 = 7 * 3 * 3 * self.conv2.weight.shape[1]
        bw2, T_w2 = self.my_quantize_7(weight2, 3)

        activation_q2 = torch.floor((torch.round(input * 1023 / T_a2) + err2) * sw_conv2)
        bb2 = torch.round(self.my_quantize(bias2, 14) * 14 * 1023 / T_a2 * 7 / T_w2)
    ...
    def forward(self, x):
        out, err, sw_conv1 = self.conv1(x)
        sw_conv1 = sw_conv1.view(1, sw_conv1.shape[0], 1, 1)
        err_conv1 = 0
        out, T_a, T_w = self.my_bn1(out, sw_conv1, err_conv1)

        if (self.layer_idx == 1) and ((self.idx == 1) or (self.idx == 2)):
            # print(1023 / T_a *7/T_w, 14 *1023/ T_a *7/T_w)
            if self.idx == 1:
                short_cut = 1023 / T_a * 7 / T_w
            else:
                short_cut = 1023 / T_a * 7 / T_w

            out += self.shortcut(x * short_cut)

            if self.idx == 1:
                out = torch.round(F.hardtanh(out, min_val=-(14 * 1023 / T_a * 7 / T_w),
                                             max_val=(14 * 1023 / T_a * 7 / T_w)) / (
                                          14 * 1023 / T_a * 7 / T_w) * 7)
            else:
                out = torch.round(F.hardtanh(out, min_val=-(14 * 1023 / T_a * 7 / T_w),
                                             max_val=(14 * 1023 / T_a * 7 / T_w)) / (
        ...
        elif (self.layer_idx == 2) and ((self.idx == 1) or (self.idx == 2)):
            # print(1023/T_a*7/T_w, 14*1023/T_a*7/T_w)
            if self.idx == 1:
                short_cut = 1023 / T_a * 7 / T_w
            else:
        ...
        elif (self.layer_idx == 3) and ((self.idx == 1) or (self.idx == 2)):
            # print(1023/T_a*7/T_w, 14*1023/T_a*7/T_w)
            if self.idx == 1:
                short_cut = 1023 / T_a * 7 / T_w
            else:
        ...
        x1 = out
        out, err2, sw_conv2 = self.conv2(x1)
        err_conv2 = 0
        sw_conv2 = sw_conv2.view(1, sw_conv2.shape[0], 1, 1)
        out, T_a2, T_w2 = self.my_bn2(out, sw_conv2, err_conv2)

        if (self.layer_idx == 1) and ((self.idx == 1) or (self.idx == 2)):
            # print(1023/T_a2*7/T_w2, 14*1023/T_a2*7/T_w2)
            if self.idx == 1:
                short_cut = 1023 / T_a2 * 7 / T_w2
            else:
                short_cut = 1023 / T_a2 * 7 / T_w2

            out = out + (x1 * short_cut)

            if self.idx == 1:
                out = torch.round(F.hardtanh(out, min_val=-(14 * 1023 / T_a2 * 7 / T_w2),
                                             max_val=(14 * 1023 / T_a2 * 7 / T_w2)) / (
                                          14 * 1023 / T_a2 * 7 / T_w2) * 7)
            else:
                out = torch.round(F.hardtanh(out, min_val=-(14 * 1023 / T_a2 * 7 / T_w2),
                                             max_val=(14 * 1023 / T_a2 * 7 / T_w2)) / (
                                          14 * 1023 / T_a2 * 7 / T_w2) * 7)
...
class ResNet(nn.Module):
    ...
    def my_bn(self, input, T, sw_conv1, err):
        ...
        activation_q_test = torch.round(input * sw_conv1 * 1023 / T_a)

        bb = torch.round(self.my_quantize(bias, 14) * 7 / T_w * 7 * 1023 / T_a)

        out = F.conv2d(activation_q_test, bw, bb, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        out = out0.view(1, 64, 1, 1)

        out, T_a2, T_w2 = self.my_bn2(out)
        
        out = F.hardtanh(out, min_val=-(7 * 64 * 1023 / T_a2 * 7 / T_w2),
